import torch 
from transformer_heads import FeedForward
...
class DecisionTransformer(torch.nn.Module):
    ...
    def __init__(self, params):
        ...
        self.predict_state = torch.nn.Linear(self.n_embed, state_n)
        self.predict_action = torch.nn.Sequential(
            torch.nn.Linear(self.n_embed, action_n), 
            torch.nn.Tanh()
        )
        self.predict_return = torch.nn.Linear(self.n_embed, 1)

        self.attn_head = SingleHead(self.n_embed, self.n_embed).to(self.device)
        self.ffn       = FeedForward(self.n_embed)
        

    def forward(self, states, actions, rewards, returns_to_go, horizon_length):

        # batch, horizon_length, state_n

        pos_em = self.pos_embedding(torch.arange(0, horizon_length))
        ...
        stack_input = torch.stack([state_em, action_em, reward_em], dim=1)   # batch_size, horizon, return+state+action - nembed
